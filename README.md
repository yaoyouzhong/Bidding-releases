...
复用已确认的姓名和证书编号，批量打开独立核验页面；按人员显示验证码处理、手机扫码和人工接手进展。

![学信网批量核验：自动识别、等待扫码与人工接手的逐人进度](assets/screen-chsi-progress.png)

结果页出现后，可检查并批量保存截图；尚未完成验证的人员单独列出，完成后可再次检查。

![学信网截图处理：已保存和跳过的逐人明细](assets/screen-chsi-results.png)

<sub>以上为实际前端的合成状态演示，姓名、证书信息和处理结果均为演示数据；未连接学信网，不是官网核验成功证明。需要扫码或人工验证码时，仍须由用户完成。</sub>
