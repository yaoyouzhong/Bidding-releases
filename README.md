...
本地处理无需调用在线 AI 服务；企业信用、学信网等官网查询，以及程序下载与更新仍需要联网。
...
在 Windows 10 / 11 64 位电脑上下载安装包，首次安装请保持联网。文字识别所需文件已内置，无需另行配置，也不必先接入 AI；官网查询需要 Google Chrome。
...
材料默认保存在本机。AI 增强默认关闭，明确开启相应用途后，相关内容才发送给所配置的模型。官网查询按用户发起的操作访问对应网站，请按业务需要备份重要资料。
