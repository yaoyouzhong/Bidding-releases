...
[Gitee 国内分卷下载](https://gitee.com/yaoyouzhong/Bidding-releases/releases/tag/v0.4.2)：下载全部分卷和配套脚本到同一文件夹，双击 install.cmd，校验合并后安装。GitHub 提供完整 EXE。分卷不能直接用于应用内更新；无法访问 GitHub 时请手动安装。
...
0.4.2 的识别模型与程序分开提供。请在应用设置中点击“下载模型”，或从本版发布页下载文档、验证码 ZIP 模型包后点击“导入模型”，无需解压。一个 ZIP 可包含多个模型文件，也可选择多个 ONNX 文件。已有匹配模型显示就绪时无需重复导入。离线导入后即可进行本地识别；详见[模型说明](TECHNICAL_NOTES.md#ocr-模型)。
...
当前安装包尚无 Windows 发布者签名。请确认文件来自本项目的 GitHub 或 Gitee 发布页；如需核对下载文件，见[安装环境与下载校验](TECHNICAL_NOTES.md#安装与下载渠道)。无法确认来源时不要运行。
